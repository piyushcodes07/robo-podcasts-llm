...
def mock_source_docs():
    ...
    youtube_doc.extract = AsyncMock(return_value='youtube content') # Make extract an AsyncMock

    pdf_doc = Mock() 
    pdf_doc.as_langchain_document.return_value = Document(page_content='pdf content')
    pdf_doc.extract = AsyncMock(return_value='pdf content') # Make extract an AsyncMock

    web_doc = Mock()
    web_doc.as_langchain_document.return_value = Document(page_content='web content')
    web_doc.extract = AsyncMock(return_value='web content') # Make extract an AsyncMock

    audio_doc = Mock()
    audio_doc.as_langchain_document.return_value = Document(page_content='audio content')
    audio_doc.extract = AsyncMock(return_value='audio content') # Make extract an AsyncMock

    word_doc = Mock()
    word_doc.as_langchain_document.return_value = Document(page_content='word content')
    word_doc.extract = AsyncMock(return_value='word content') # Make extract an AsyncMock
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_youtube(mock_source_docs):
    ...
    with patch('podcast_llm.extractors.utils.YouTubeSourceDocument', return_value=mock_source_docs['youtube']):
        ...
        result = await utils.extract_content_from_sources(sources)
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_pdf(mock_source_docs):
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_web(mock_source_docs):
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_audio(mock_source_docs):
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_docx(mock_source_docs):
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_multiple(mock_source_docs):
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_failure():
    """Test handling of extraction failures"""
    with patch('podcast_llm.extractors.utils.YouTubeSourceDocument') as mock_youtube:
        mock_youtube.return_value.extract.side_effect = Exception('Extraction failed') # Access extract method of the mock instance
        sources = ['https://youtube.com/watch?v=123']
        result = await utils.extract_content_from_sources(sources)
...
@pytest.mark.asyncio
async def test_extract_content_from_sources_empty():
    """Test handling of empty source list"""
    result = await utils.extract_content_from_sources([])
    assert len(result) == 0